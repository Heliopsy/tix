...
func TestDependencyPathQueryRefusesWithoutScope(t *testing.T) {
	if _, _, err := sqlb.DependencyPathQuery(sqlb.SQLite, core.TenantScope{}, "a", "b"); !core.IsKind(err, core.KindInvalid) {
		t.Fatalf("unscoped dependency walk = %v, want invalid", err)
	}
	q, args, err := sqlb.DependencyPathQuery(sqlb.SQLite, core.TenantScope{TenantID: "t1"}, "a", "b")
	if err != nil {
		t.Fatalf("scoped dependency walk: %v", err)
	}
	if len(args) != 4 || args[0] != "t1" {
