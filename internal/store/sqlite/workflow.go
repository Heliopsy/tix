...
func scanWorkflow(s scanner) (core.Workflow, error) {
	...
	if err := s.Scan(&w.ID, &w.TenantID, &w.Key, &w.Name, &definition, &builtin,
		&created, &updated); err != nil {
		return core.Workflow{}, mapErr(err, "scanning workflow")
	}
	w.Builtin = sqlb.Bool(builtin)
	...
	var err error
	if w.CreatedAt, err = sqlb.ScanTime(created); err != nil {
		return core.Workflow{}, err
	}
	if w.UpdatedAt, err = sqlb.ScanTime(updated); err != nil {
		return core.Workflow{}, err
	}
	return w, nil
}
...
func (t *tx) PutWorkflow(ctx context.Context, w *core.Workflow) error {
	...
	upd := t.builder("workflows").
		...
		Set("builtin", sqlb.BoolInt(w.Builtin)).
		Set("updated_at", sqlb.TimeText(now))
	...
	ins := t.insert("workflows").
		...
		Set("definition", definition).
		Set("builtin", sqlb.BoolInt(w.Builtin)).
		Set("created_at", sqlb.TimeText(w.CreatedAt)).
		Set("updated_at", sqlb.TimeText(w.UpdatedAt))
