...
func (t *tx) statsCompleted(ctx context.Context, q store.StatsQuery) ([]store.CompletedTask, error) {
	b := t.builder("tasks").
		...
		Where("tasks.completed_at >= ?", timeArg(q.Since)).
		Where("tasks.completed_at <= ?", timeArg(q.Until)).
	...
	for rows.Next() {
		...
		row.ActorID = text(actor)
		row.CreatedAt = scanTime(created)
		row.CompletedAt = scanTime(completed)
		out = append(out, row)
	}
...
func (t *tx) statsCreated(ctx context.Context, q store.StatsQuery) (int, error) {
	b := t.builder("tasks").
		Where("tasks.deleted_at IS NULL").
		Where("tasks.created_at >= ?", timeArg(q.Since)).
		Where("tasks.created_at <= ?", timeArg(q.Until))
	statsProject(b, q)
