...
func scanEvent(s scanner) (core.Event, error) {
	...
	e.ProjectID = text(project)
	e.ActorID = text(actor)
	if err := sqlb.ParseJSON(payload, &e.Payload); err != nil {
		return core.Event{}, core.Internal("decoding payload of event %q", e.ID).Wrap(err)
	}
	e.OccurredAt = scanTime(occurred)
	return e, nil
}

// AppendEvent writes to the outbox in the caller's transaction and arms the
// notification this transaction sends on commit.
func (t *tx) AppendEvent(ctx context.Context, e *core.Event) error {
	...
	ins := t.insert("events").
		...
		Set("project_id", nullText(e.ProjectID)).
		Set("subject_type", e.SubjectType).
		Set("subject_id", e.SubjectID).
		Set("actor_id", nullText(e.ActorID)).
		Set("payload", payload).
		Set("occurred_at", timeArg(e.OccurredAt))
	if err := t.insertReturning(ctx, ins, "seq", &e.Seq,
		"appending event %q", string(e.Type)); err != nil {
		return err
	}
	t.notify = true
	return nil
...
func (t *tx) AppendAudit(ctx context.Context, e *core.AuditEntry) error {
	...
	ins := t.insert("audit_entries").
		...
		Set("subject_type", e.SubjectType).
		Set("subject_id", e.SubjectID).
		Set("before_state", nullText(string(e.Before))).
		Set("after_state", nullText(string(e.After))).
		Set("source", string(e.Source)).
		Set("occurred_at", timeArg(e.OccurredAt))
	return t.insertReturning(ctx, ins, "seq", &e.Seq, "appending audit entry %q", e.Action)
}
...
func (t *tx) ListAudit(ctx context.Context, f core.AuditFilter) ([]core.AuditEntry, error) {
	...
	if f.Since != nil {
		b.Where("occurred_at >= ?", timeArg(*f.Since))
	}
	if f.Until != nil {
...
func (t *tx) PruneEvents(ctx context.Context, before time.Time, floorSeq int64, limit int) (int64, error) {
	...
	inner := t.embed("events").
		Select("seq").
		Where("occurred_at < ?", timeArg(before)).
		Where("seq <= ?", floorSeq).
		OrderBy("seq", core.Ascending).
		Limit(limit)
...
func (t *tx) PruneDeliveries(ctx context.Context, before time.Time, limit int) (int64, error) {
	...
	inner := t.embed("webhook_deliveries").
		Select("id").
		Where("created_at < ?", timeArg(before)).
