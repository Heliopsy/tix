...
func scanUser(s scanner) (core.User, error) {
	...
	u.DisabledAt = scanNullTime(disabled)
	return u, nil
}
...
func (t *tx) CreateUser(ctx context.Context, u *core.User, passwordHash string) error {
	...
	ins := t.insert("users").
		...
		Set("created_at", timeArg(u.CreatedAt)).
		Set("updated_at", timeArg(u.UpdatedAt)).
		Set("disabled_at", nullTimeArg(u.DisabledAt)).
...
func (t *tx) CreateSession(ctx context.Context, actorID, tokenHash string, expiresAt time.Time) error {
	ins := t.insert("sessions").
		...
		Set("created_at", t.now()).
		Set("expires_at", timeArg(expiresAt))
...
func (t *tx) GetSessionByHash(ctx context.Context, tokenHash string) (string, time.Time, error) {
	...
	return actorID, scanTime(expires), nil
}
...
func (t *tx) DeleteExpiredSessions(ctx context.Context, now time.Time) (int64, error) {
	b := t.builder("sessions").Where("expires_at <= ?", timeArg(now))
...
func scanToken(s scanner) (core.APIToken, error) {
	...
	tk.CreatedAt = scanTime(created)
	tk.ExpiresAt = scanNullTime(expires)
	tk.LastUsedAt = scanNullTime(lastUsed)
	tk.RevokedAt = scanNullTime(revoked)
	return tk, nil
}
...
func (t *tx) CreateToken(ctx context.Context, tk *core.APIToken, tokenHash string) error {
	...
	ins := t.insert("api_tokens").
		...
		Set("token_hash", tokenHash).
		Set("scopes", scopes).
		Set("project_id", nullText(tk.ProjectID)).
		Set("created_at", timeArg(tk.CreatedAt)).
		Set("expires_at", nullTimeArg(tk.ExpiresAt)).
		Set("last_used_at", nullTimeArg(tk.LastUsedAt)).
		Set("revoked_at", nullTimeArg(tk.RevokedAt))
