...
- [x] 4.16 WP-32 Transport-equivalence suite covering every operation on the service contract against both implementations (`internal/integration/service_matrix_test.go`; `service_matrix_coverage_test.go` reflects over `capability.Operations()` and fails the build for any operation with neither a scenario nor a written exemption)
...
- [x] 7.3 WP-50 Parity test: every operation has CLI, HTTP, and Web bindings, or a recorded exemption (six CLI bindings are absent behind `Exemption.Gap` markers in `internal/capability/registry.go`; the test accepts them as known defects, not as justifications)
