...
- [ ] 4.16 WP-32 Transport-equivalence suite running every scenario against both implementations (no suite exists; `internal/httpapi/handlers_task_test.go` is the only test that drives a `service.Local` and a `client.Client` through the same case, and it covers custom-field filtering alone)
